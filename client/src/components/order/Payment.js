...
export default function Payment() {
  ...
  const getTotalOrder = () => {
    let TotalSalePrice = 0, total = 0;
    if (cartItems) {
      cartItems.forEach(item => {
        console.log(item.salePrice);
        TotalSalePrice += parseFloat(item.salePrice);
        console.log(TotalSalePrice);
      });
      total = (TotalSalePrice / 24000).toFixed(2);
      console.log(total);
    }
    // Chỉ cần gọi setTotalSalePrice với giá trị total
